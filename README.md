...
> Только для пользователей с верифицированным аккаунтом в reg.cloud
...
Нужна **одна строка Cookie**, как в заголовке запроса из браузера (начинается с **`regru_utr=`** или **`_ym_uid= `** и по смыслу содержит сессию; в конце цепочки обычно фигурирует **`jwt_refresh`** — это нормальный признак актуальной связки куков).

> [!WARNING]
> После вставки токена в конфиг ОБЯЗАТЕЛЬНО закройте вкладку с reg.ru или вообще браузер, иначе сессия дропнется.
...
7. Скопируйте **значение целиком** — длинную строку, которая **начинается с `regru_utr=` или `_ym_uid= `** и включает все пары `имя=значение` через **`; `**.  
